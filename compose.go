// Package compose provides generic utilities for building nested data structures
...
// ExtractIDs collects the foreign keys needed between levels.
//
...
//
// It pre-allocates the output slice via append, so mapFunc may return zero values
// to skip elements without shifting — the caller can filter afterward if needed.
//
